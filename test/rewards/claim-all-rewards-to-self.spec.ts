...
import { HTokenMock__factory } from '../../types';
...
makeSuite('Incentives Controller V2 claimRewards to self tests', (testEnv) => {
  ...
  for (const { caseName, emissionsPerSecond, zeroBalance } of getRewardsBalanceScenarios) {
    it(caseName, async () => {
      ...
      const assets = [hDaiMockV2, hHopeMockV2, hWethMockV2].map(({ address }) => address);
      ...
      await Bluebird.each(assets, async (asset, index) => {
        await HTokenMock__factory.connect(asset, deployer.signer).setUserBalanceAndSupply(
          stakedByUser[index],
          totalSupply[index]
        );
      });
      ...
      await Bluebird.each(assets, async (asset, index) => {
        await HTokenMock__factory.connect(asset, deployer.signer).handleActionOnAic(
          userAddress,
          totalSupply[index],
          stakedByUser[index]
        );
      ...
      await hDaiMockV2.cleanUserState();
