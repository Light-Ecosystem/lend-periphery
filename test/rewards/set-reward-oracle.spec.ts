...
import { deployMockAggregator, ZERO_ADDRESS } from 'lend-deploy';
import { hrtime } from 'process';
import { parseEther } from '@ethersproject/units';

const { expect } = require('chai');

makeSuite('HopeLendIncentivesControllerV2 setRewardOracle', (testEnv: TestEnv) => {
  ...
  it('Revert at setRewardOracle if oracle price is zero', async () => {
    const { rewardsController, deployer, hopeToken } = testEnv;

    const zeroPriceOracle = await deployMockAggregator('0');

    await expect(
      rewardsController.setRewardOracle(hopeToken.address, zeroPriceOracle.address)
