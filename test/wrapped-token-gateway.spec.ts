...
makeSuite('Use native ETH at Pool via WrappedTokenGateway', (testEnv: TestEnv) => {
  const zero = BigNumber.from('0');
  const depositSize = utils.parseEther('5');
  const daiSize = utils.parseEther('10000');
  it('Deposit WETH via WrappedTokenGateway and DAI', async () => {
    ...
    await WrappedTokenGateway.connect(depositor.signer).depositETH(
      pool.address,
      depositor.address,
      '0',
      { value: depositSize }
    ...
    await WrappedTokenGateway.connect(user.signer).depositETH(pool.address, user.address, '0', {
      value: depositSize,
    });
  ...
  it('Withdraw WETH - Partial', async () => {
    ...
    const { gasUsed: withdrawGas } = await waitForTx(
      await WrappedTokenGateway.connect(user.signer).withdrawETH(
        pool.address,
        partialWithdraw,
        user.address
      )
    ...
    expect(afterPartialHTokensBalance).to.be.equal(
      ...
      'User hWETH balance should be substracted'
    );
  });
  ...
  it('Withdraw WETH - Full', async () => {
    ...
    expect(afterFullHTokensBalance).to.be.eq(0, 'User hWETH balance should be zero');
  });

  it('Borrow stable WETH and Full Repay with ETH', async () => {
    const { users, WrappedTokenGateway, hDai, weth, dai, pool, helpersContract } = testEnv;
    ...
    const hTokensBalance = await hDai.balanceOf(user.address);

    expect(hTokensBalance).to.be.gt(zero);
    expect(hTokensBalance).to.be.gte(daiSize);
    ...
    await waitForTx(
      await WrappedTokenGateway.connect(user.signer).repayETH(
        pool.address,
        MAX_UINT_AMOUNT,
        '1',
        user.address,
    ...
    // Withdraw DAI
    await hDai.connect(user.signer).approve(pool.address, MAX_UINT_AMOUNT);
  ...
  it('Borrow variable WETH and Full Repay with ETH', async () => {
    const { users, WrappedTokenGateway, hWETH, weth, pool, helpersContract } = testEnv;
    const borrowSize = utils.parseEther('1');
    const repaySize = borrowSize.add(borrowSize.mul(5).div(100));
    const user = users[1];
  ...
  it('Borrow ETH via delegateApprove ETH and repays back', async () => {
    const { users, WrappedTokenGateway, hWETH, weth, helpersContract, pool } = testEnv;
    const borrowSize = utils.parseEther('1');
    const user = users[2];
    const { variableDebtTokenAddress } = await helpersContract.getReserveTokensAddresses(
      weth.address
    ...
    await waitForTx(
      await WrappedTokenGateway.connect(user.signer).borrowETH(pool.address, borrowSize, '2', '0')
    );
  ...
  it('Owner can do emergency token recovery', async () => {
    const { users, dai, WrappedTokenGateway, deployer } = testEnv;
    const user = users[0];
    const amount = utils.parseEther('1');
    ...
    await dai.connect(user.signer).transfer(WrappedTokenGateway.address, amount);
    ...
    await WrappedTokenGateway.connect(deployer.signer).emergencyTokenTransfer(
      dai.address,
      user.address,
      amount
    );
  ...
  it('Owner can do emergency native ETH recovery', async () => {
    const { users, WrappedTokenGateway, deployer } = testEnv;
    const user = users[0];
    const amount = utils.parseEther('1');
    ...
    // Selfdestruct the mock, pointing to WrappedTokenGateway address
    const callTx = await selfdestructContract
      .connect(user.signer)
      .destroyAndTransfer(WrappedTokenGateway.address, { value: amount });
    ...
    await WrappedTokenGateway.connect(deployer.signer).emergencyEtherTransfer(
      user.address,
      amount
    );

    const userBalanceAfterRecovery = await user.signer.getBalance();
    const WrappedTokenGatewayAfterRecovery = await hre.ethers.provider.getBalance(
    ...
    expect(WrappedTokenGatewayAfterRecovery).to.be.eq(
      '0',
      'WrappedTokenGateway ether balance should be zero.'
    );
  });
});
